...
export class DigitOnlyDirective implements OnChanges {
  ...
  @Input() numberOfIntegers = Infinity;
  ...
  @Input() max = Infinity;
  @Input() currency = "";
  @Input() pattern?: string | RegExp = "^\\d+(?:[,]\\d+)?$";
  ...
  onBlur(event: FocusEvent): void {
    let textData: string = (event.target as HTMLInputElement).value;
    if (!this.isNumeric(textData.replace("$", "").replace(/\./g, '').replace(",", ".").trim())) {
      this.inputElement.value = "0";
      return;
    }

    textData = limitNumber({ value: textData, limit: this.numberOfIntegers });
    textData = maxMinNumber({ value: textData, decimales: this.numberOfDecimals, min: this.min, max: this.max });
  ...
  onFocus(event: FocusEvent): void {
    const textData: string = (event.target as HTMLInputElement).value;
